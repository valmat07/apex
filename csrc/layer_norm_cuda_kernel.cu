...
template<typename T, typename U> __device__
void cuWelfordMuSigma2Optimized(
  const T* __restrict__ vals,
  const int n1,
  const int n2,
  const int i1,
...
template<typename T, typename U> __device__
void cuWelfordMuSigma2OptimizedFused(
  const T* __restrict__ vals,
  ...
  U* buf)
{
  ...
  if (i1 < n1) {
    ...
    for (;  l+3 < n2;  l+=4*numx) {
      for (int k = 0;  k < 4;  ++k) {
        U curr = static_cast<U>(lvals[l+k]);
        U curr_res = static_cast<U>(lresidual[l+k]);
        cuRMSOnlineSum<U>(curr + curr_res, sigma2);
      }
    }
    for (;  l < n2;  ++l) {
      U curr = static_cast<U>(lvals[l]);
      U curr_res = static_cast<U>(lresidual[l]);
      cuRMSOnlineSum<U>(curr + curr_res, sigma2);
    }
    // intra-warp reductions
    for (int l = 0;  l <= 4;  ++l) {
  ...
  const V* __restrict__ gamma)
{
  ...
  for (auto i1=blockIdx.y; i1 < n1; i1 += gridDim.y) {
    ...
    U* buf = shared.getPointer();
    U mu,sigma2;
    cuWelfordMuSigma2Optimized(vals,n1,n2,i1,mu,sigma2,buf);
...
template<typename T, typename U, typename V> __device__
void cuApplyRMSNormOptimizedFused_(
  V* __restrict__ output_vals,
  U* __restrict__ invvar,
  const T* __restrict__ vals,
  const T* __restrict__ residual,
  ...
  const V* __restrict__ gamma)
{
  ...
  for (auto i1=blockIdx.y; i1 < n1; i1 += gridDim.y) {
    ...
    U* buf = shared.getPointer();
    U mu,sigma2;
    cuWelfordMuSigma2OptimizedFused(vals, residual, n1,n2,i1,mu,sigma2,buf);
...
{
  if (residual != NULL) {
    cuApplyRMSNormOptimizedFused_<T, U, V>(output_vals, invvar, vals, residual, n1, n2, epsilon, gamma);
  }
  else {
    cuApplyRMSNormOptimized_<T, U, V>(output_vals, invvar, vals, n1, n2, epsilon, gamma);
  }
}
...
template<typename T, typename U, typename V, bool MemoryEfficient> __device__
void cuLoadWriteStridedInputsOptimized(
    const int i1_block,
    const int thr_load_row_off,
    const int thr_load_col_off,
    const int i2_off,
...
template<typename T, typename U, typename V, bool MemoryEfficient> __device__
void cuLoadWriteStridedInputsOptimizedFused(
    const int i1_block,
    const int thr_load_row_off,
    const int thr_load_col_off,
    const int i2_off,
  ...
  if (i1 < i1_end) {
    for (int k = 0;  k < blockDim.y;  ++k) {
      ...
      if (i2<n2) {
        ...
        U curr_dout = static_cast<U>(dout[load_idx]);
        if (MemoryEfficient) {
          warp_buf2[write_idx] = curr_dout * (c_h + res_h) / static_cast<U>(clamp_by_magnitude(gamma[i2], eps));
        } else {
...
template<typename T, typename U, typename V, bool MemoryEfficient> __device__
void cuLoadAddStridedInputsOptimized(
    const int i1_block,
    const int thr_load_row_off,
    const int thr_load_col_off,
    const int i2_off,
...
template<typename T, typename U, typename V, bool MemoryEfficient> __device__
void cuLoadAddStridedInputsOptimizedFused(
    const int i1_block,
    const int thr_load_row_off,
    const int thr_load_col_off,
    const int i2_off,
...
template<typename T, typename U, typename V, bool MemoryEfficient> __global__
void cuComputePartGradGammaBetaOptimized(
    const V* __restrict__ dout,
    const T* __restrict__ input_or_output,
    const int n1,
    const int n2,
    ...
    const double eps)
{
    ...
    // compute partial sums from strided inputs
    // do this to increase number of loads in flight
    cuLoadWriteStridedInputsOptimized<T, U, V, MemoryEfficient>(i1_beg,thr_load_row_off,thr_load_col_off,i2_off,row_stride,warp_buf1,warp_buf2,input_or_output,dout,i1_end,n2, invvar,gamma, eps);
    for (int i1_block = i1_beg+blockDim.y*blockDim.y;  i1_block < i1_end;  i1_block+=blockDim.y*blockDim.y) {
      cuLoadAddStridedInputsOptimized<T, U, V, MemoryEfficient>(i1_block,thr_load_row_off,thr_load_col_off,i2_off,row_stride,warp_buf1,warp_buf2,input_or_output,dout,i1_end,n2,invvar,gamma, eps);
    }
    __syncthreads();
    // inter-warp reductions
    // sum within each warp
    ...
    const double eps)
{
    ...
    // compute partial sums from strided inputs
    // do this to increase number of loads in flight
    cuLoadWriteStridedInputsOptimizedFused<T, U, V, MemoryEfficient>(i1_beg,thr_load_row_off,thr_load_col_off,i2_off,row_stride,warp_buf1,warp_buf2,input_or_output, residual, dout,i1_end,n2, invvar,gamma, eps);
    for (int i1_block = i1_beg+blockDim.y*blockDim.y;  i1_block < i1_end;  i1_block+=blockDim.y*blockDim.y) {
      cuLoadAddStridedInputsOptimizedFused<T, U, V, MemoryEfficient>(i1_block,thr_load_row_off,thr_load_col_off,i2_off,row_stride,warp_buf1,warp_buf2,input_or_output, residual, dout,i1_end,n2,invvar,gamma, eps);
    }
    __syncthreads();
    // inter-warp reductions
    // sum within each warp
...
template<typename U, typename V> __global__
void cuComputeGradGammaBetaOptimized(
    const U* part_grad_gamma,
    const int part_size,
    const int n1,
    const int n2,
...
template<typename T, typename U, typename V, bool MemoryEfficient> __global__
void cuComputeGradInputOptimized(
    const V* __restrict__ dout,
    const T* __restrict__ input_or_output,
    const int n1,
    const int n2,
    ...
    const double eps)
{
  for (auto i1=blockIdx.y; i1 < n1; i1 += gridDim.y) {
    ...
    for (;  l+3 < n2;  l+=4*numx) {
      for (int k = 0;  k < 4;  ++k) {
        const U c_h = static_cast<U>(k_h[l+k] + res_h[l+k]);
        const U c_loss = static_cast<U>(k_dout[l+k]);
    ...
    for (;  l < n2;  ++l) {
      const U c_h = static_cast<U>(k_h[l] + res_h[l]);
      const U c_loss = static_cast<U>(k_dout[l]);
    ...
    U term1 = (U(1) / fH) * c_invvar;
    T* k_grad_input = grad_input + i1*n2;
    T* k_res_input = grad_input + i1*n2;
    for (int l = thrx;  l < n2;  l+=numx) {
      const U c_h = static_cast<U>(k_h[l] + res_h[l]);
      const U c_loss = static_cast<U>(k_dout[l]);
      const U k_gamma = static_cast<U>(clamp_by_magnitude(gamma[l], eps));
      ...
      f_grad_input *= term1;
      k_grad_input[l] = static_cast<T>(f_grad_input);
      k_res_input[l] = static_cast<T>(f_grad_input);
    }
...
template<typename T, typename U, typename V=T>
void HostApplyRMSNormOptimized(
    V* output,
    U* invvar,
    const T* input,
    const T* residual,
    ...
    const V* gamma)
{
    ...
    cuApplyRMSNormOptimized<<<blocks, threads, nshared, stream>>>(
      output, invvar, input, residual, n1, n2, U(epsilon), gamma);
}
    ...
    double epsilon)
{
    using namespace at;
    DISPATCH_DOUBLE_FLOAT_HALF_AND_BFLOAT_INOUT_TYPES(
        input->scalar_type(), output->scalar_type(), "rms_norm_cuda_kernel_optimized",
        using accscalar_t = at::acc_type<scalar_t_in, true>;
        HostApplyRMSNormOptimized<scalar_t_in, accscalar_t, scalar_t_out>(
          output->DATA_PTR<scalar_t_out>(),
          invvar->DATA_PTR<accscalar_t>(),
          input->DATA_PTR<scalar_t_in>(),
...
template<typename T, typename U=float, typename V=T>
void HostRMSNormGradientOptimized(
    const V* dout,
    const U* invvar,
    at::Tensor* input_or_output,
    at::Tensor* residual,
    ...
    bool memory_efficient)
{
    ...
    if (gamma != NULL) {
      ...
      BOOL_SWITCH(memory_efficient, MemoryEfficient, [&]{
        if (residual != NULL) {
          auto kernel = &cuComputePartGradGammaBetaOptimizedFused<T, U, V, MemoryEfficient>;
          kernel<<<blocks2, threads2, nshared2, stream>>>(
                          dout,
                          input_or_output->DATA_PTR<T>(),
                          residual->DATA_PTR<T>(),
          ...
          auto kernel = &cuComputePartGradGammaBetaOptimized<T, U, V, MemoryEfficient>;
          kernel<<<blocks2, threads2, nshared2, stream>>>(
                          dout,
                          input_or_output->DATA_PTR<T>(),
                          n1,n2,
    ...
    BOOL_SWITCH(memory_efficient, MemoryEfficient, [&] {
      if (residual != NULL) {
        auto kernel = cuComputeGradInputOptimizedFused<T, U, V, MemoryEfficient>;
        kernel<<<blocks1, threads1, nshared, stream>>>(
                dout,
                input_or_output->DATA_PTR<T>(),
                residual->DATA_PTR<T>(),
        ...
        auto kernel = cuComputeGradInputOptimized<T, U, V, MemoryEfficient>;
        kernel<<<blocks1, threads1, nshared, stream>>>(
                dout,
                input_or_output->DATA_PTR<T>(),
                n1,n2,
...
void cuda_rms_norm_gradient_optimized(
    at::Tensor* dout,
    at::Tensor* invvar,
    at::Tensor* input_or_output,
    at::Tensor* residual,
